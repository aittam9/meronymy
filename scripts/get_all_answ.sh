...
if [[ $# -ne 2 ]]; then
	...
	echo "  data: mcrae | mcrae_lemma | conceptnet | all"
...
case "$DATA" in
	mcrae|mcrae_lemma|conceptnet|all) ;;
	*)
		echo "Invalid data: $DATA"
		echo "Expected one of: mcrae, mcrae_lemma, conceptnet, all"
		exit 1
		;;
esac
...
if [[ "$DATA" == "all" ]]; then
	DATASETS=("mcrae" "mcrae_lemma" "conceptnet")
else
	DATASETS=("$DATA")
fi
...
for DATA_ITEM in "${DATASETS[@]}"; do
	for MODEL in "${MODELS[@]}"; do
		...
		"$PYTHON_BIN" get_answers.py --data "$DATA_ITEM" --task "$TASK" --model "$MODEL"
	done
done
