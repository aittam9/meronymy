...
def prompt_format(sent, task, nodes) -> str:
    """
    ...
    if task == "questions":
        ...
        return f"Consider the following entities:\nEntity1: {n1}\nEntity2: {n2}\nAnswer yes or no to the following question: {sent} \nAnswer: "
    else:
        #return f"Judge if the following statement is true or false: {sent} \nAnswer: "
        return f"Consider the following entities:\nEntity1: {n1}\nEntity2: {n2}\nJudge if the following statement is true or false: {sent} \nAnswer: "
    # Consider the following two entities: {e1} and {e2} and answer yes or no to the following question: {sent} \nAnswer: "
...
if __name__ == "__main__":
    ...
    print(f"\nExample prompt:\n{prompt_format(sents[0], args.task, nodes[0])}\n")
    #feeding the model with the chosen inputs
    answers = [model(prompt_format(s, args.task, n), output_type, max_new_tokens=10, temperature=0., do_sample=False)   for s, n in tqdm(list(zip(sents, nodes))) if s] 
    # testing swapped 
    print(f"\nStarting feeding the model with {args.task} swapped\n")
    print(f"Example prompt swapped:\n{prompt_format(swapped[0], args.task, nodes[0])}\n")
    answer_swapped = [model(prompt_format(s, args.task, n), output_type, max_new_tokens=10, temperature=0., do_sample=False) for s, n in tqdm(list(zip(swapped, nodes))) if s]
